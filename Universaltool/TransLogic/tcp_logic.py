...
class TcpLogic(Signal.Signals):
    ...
    def get_ip_tcp(self):
        ...
        try:
            ...
            self.signal_write_terminal.emit("自动获取IP "+str(my_addr)+"\n")
        except Exception as ret:
            # 若无法连接互联网使用，会调用以下方法
            try:
                my_addr = socket.gethostbyname(socket.gethostname())
            except Exception as ret_e:
                self.signal_write_terminal.emit("无法获取ip，请连接网络！\n")
        finally:
            s.close()
        return my_addr
        ...
        if self.tcp_link is False:
            ...
            try:
                if self.CurrentClient == -1:
                    ...
                    client, address = self.tcp_client_socket_list[self.CurrentClient]
                    client.send(sendstr)
                    msg = 'TCP客户端已发送\n'
                    self.signal_write_terminal.emit(msg)
